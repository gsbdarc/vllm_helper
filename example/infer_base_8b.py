#!/usr/bin/env python3
# infer_base_8b.py — base model only (no adapter)
# - EXACT training-format prompts via /v1/completions
# - Tries guided_choice to constrain output to VALID LABELS (falls back if unsupported)
# - Prints running accuracy + UNKNOWN count at checkpoints; saves preds/unknowns/metrics
...
def load_jsonl(path: str):
    ...
    with open(path, "r", encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            rows.append({
                "id": o.get("id", i),
                "prompt": o["prompt"],         # EXACT training prompt (ends with 'Answer:')
...
def assert_model_listed(base_url: str, model_id: str):
    try:
        r = requests.get(f"{base_url}/v1/models", timeout=10)
        r.raise_for_status()
        ids = [m.get("id") for m in (r.json().get("data") or [])]
        if model_id in ids:
            print(f"[ok] Base model '{model_id}' is listed in /v1/models.")
        else:
            print(f"[WARN] Base model '{model_id}' not found in /v1/models. Found: {ids}")
...
def call_vllm_completions(base_url: str, model_field: str, prompt: str) -> dict:
    ...
    base_payload = {
        "model": model_field,          # base model id, e.g., "Qwen/Qwen3-8B-Base"
        "prompt": prompt,
        "temperature": 0.0,
        "top_p": 1.0,
...
def main():
    ...
    MODEL_FIELD = BASE_MODEL               # request the base model (no adapter)

    BASE = f"http://{host}:{port}"
    assert_model_listed(BASE, BASE_MODEL)

    pred_dir = f"{BASE_PRED_ROOT}/qwen_8b_base"
    ensure_dir(pred_dir)
    OUT_JSON     = f"{pred_dir}/predictions_qwen_base.json"
    UNKNOWN_JSON = f"{pred_dir}/unknown_predictions_qwen_base.json"
    CM_CSV       = f"{pred_dir}/confusion_qwen_base.csv"
    METRICS_JSON = f"{pred_dir}/metrics_qwen_base.json"

    print(f"Using vLLM completions at {host}:{port} | model={BASE_MODEL}")
    ...
    for row in tqdm(tests, total=total_examples, desc="Infer (base)"):
        rid = int(row["id"])
    ...
    # Final prints
    if acc is not None:
        print(f"Final accuracy (base): {acc:.4f} over {len(labeled)} labeled examples")
    else:
        print("Final accuracy (base): N/A (no gold labels)")
